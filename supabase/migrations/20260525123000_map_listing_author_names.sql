...
update public.maps
set author_name = coalesce(nullif(btrim(profiles.display_name), ''), 'Anonymous Cartographer')
from public.profiles
where profiles.id = maps.owner_id
  and maps.author_name is null;
...
alter table public.maps
  ...
  alter column author_name set default 'Anonymous Cartographer',
  add constraint maps_author_name_length check (char_length(author_name) <= 64);
