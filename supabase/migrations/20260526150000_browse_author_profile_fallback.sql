update public.maps
set author_name = btrim(public.profiles.display_name)
from public.profiles
where public.profiles.id = public.maps.owner_id
  and (
    public.maps.author_name is null
    or btrim(public.maps.author_name) = ''
    or public.maps.author_name = 'Anonymous Cartographer'
  )
  and nullif(btrim(public.profiles.display_name), '') is not null
  and btrim(public.profiles.display_name) <> 'Anonymous Cartographer';
...
as $$
  select
    ...
    maps.updated_at,
    coalesce(
      nullif(nullif(btrim(maps.author_name), ''), 'Anonymous Cartographer'),
      nullif(nullif(btrim(profiles.display_name), ''), 'Anonymous Cartographer'),
      nullif(btrim(maps.author_name), ''),
